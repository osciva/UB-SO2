...
#define N 1// number of lines that reads each thread
...
void *thread_function(void *args){
  INFORMATION *data = (INFORMATION *) args;
  char origin[STR_CODE_AIRPORT], destination[STR_CODE_AIRPORT];
  int invalid, index_origin, index_destination;

  char** line_matrix = (char **)malloc(sizeof(char*) * N);

  for (int i = 0; i < N; i++) line_matrix[i] = malloc(sizeof(char)*MAXCHAR);

  while(!feof(data->fp)){
    // Get N line a file
    int count = 0;
    pthread_mutex_lock(&mutex);
    while(count <  N && fgets(line_matrix[count], MAXCHAR, data->fp) != NULL){
      count ++;
    }
    pthread_mutex_unlock(&mutex);

    for(int i = 0; i < count; i++){
      invalid = extract_fields_airport(origin, destination, line_matrix[i]);
  ...
  free_matrix((void **)line_matrix, N);
...
void read_airports_data_main(int **num_flights, char **airports, char *fname){
    char line[MAXCHAR];
    ...
    INFORMATION *data = (INFORMATION *) malloc(sizeof(INFORMATION));
    data->fp = fp;
    ...
    for (int i = 0; i < NUM_THREADS; i++){
        int err = pthread_create(thread_id + i, NULL, thread_function, (void *) data);
        if (err != 0){
...
int main(int argc, char **argv)
{
  ...
  // Inicializa la matriz de vuelos a 0
  ...
  read_airports_data_main(num_flights, airports, argv[2]);
